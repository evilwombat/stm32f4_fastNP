...
DMA_HandleTypeDef hdma_tim1_pwm_ch1;
DMA_HandleTypeDef hdma_tim1_pwm_ch2;
...
static void ws2812_timer1_init(void)
{
    ...
    htimer1.Init.Period = WS2812_TIMER_PERIOD;

    htimer1.Init.ClockDivision = TIM_CLOCKDIVISION_DIV1;
    htimer1.Init.AutoReloadPreload = TIM_AUTORELOAD_PRELOAD_DISABLE;
    HAL_TIM_Base_Init(&htimer1);

    sClockSourceConfig.ClockSource = TIM_CLOCKSOURCE_INTERNAL;
    HAL_TIM_ConfigClockSource(&htimer1, &sClockSourceConfig);
    HAL_TIM_PWM_Init(&htimer1);
    sMasterConfig.MasterOutputTrigger = TIM_TRGO_RESET;
    sMasterConfig.MasterSlaveMode = TIM_MASTERSLAVEMODE_DISABLE;
    HAL_TIMEx_MasterConfigSynchronization(&htimer1, &sMasterConfig);
    ...
    HAL_TIM_PWM_ConfigChannel(&htimer1, &sConfigOC, TIM_CHANNEL_1);
    sConfigOC.OCMode = TIM_OCMODE_PWM1;

    sConfigOC.Pulse = WS2812_TIMER_PWM_CH2_TIME;
    sConfigOC.OCPolarity = TIM_OCPOLARITY_HIGH;
    sConfigOC.OCFastMode = TIM_OCFAST_DISABLE;
    HAL_TIM_PWM_ConfigChannel(&htimer1, &sConfigOC, TIM_CHANNEL_2);
}
...
static void ws2812_dma_start(GPIO_TypeDef *gpio_bank)
{
    ...
    /* TIM1 DMA Init */
    /* TIM1_UP Init  - restore everything to 1*/
    ...
    hdma_tim1_update.Init.MemInc = DMA_MINC_DISABLE;
    hdma_tim1_update.Init.PeriphDataAlignment = DMA_PDATAALIGN_HALFWORD;
    hdma_tim1_update.Init.MemDataAlignment = DMA_MDATAALIGN_HALFWORD;
    hdma_tim1_update.Init.Mode = DMA_CIRCULAR;
    hdma_tim1_update.Init.Priority = DMA_PRIORITY_VERY_HIGH;
    hdma_tim1_update.Init.FIFOMode = DMA_FIFOMODE_DISABLE;

    /* TIM1_CH1 Init  - selectively clear bits for channels that are 0 */
    hdma_tim1_pwm_ch1.Instance = DMA2_Stream1;
    hdma_tim1_pwm_ch1.Init.Channel = DMA_CHANNEL_6;
    hdma_tim1_pwm_ch1.Init.Direction = DMA_MEMORY_TO_PERIPH;
    hdma_tim1_pwm_ch1.Init.PeriphInc = DMA_PINC_DISABLE;
    hdma_tim1_pwm_ch1.Init.MemInc = DMA_MINC_ENABLE;
    hdma_tim1_pwm_ch1.Init.PeriphDataAlignment = DMA_PDATAALIGN_HALFWORD;
    hdma_tim1_pwm_ch1.Init.MemDataAlignment = DMA_MDATAALIGN_HALFWORD;
    hdma_tim1_pwm_ch1.Init.Mode = DMA_CIRCULAR;
    hdma_tim1_pwm_ch1.Init.Priority = DMA_PRIORITY_VERY_HIGH;
    hdma_tim1_pwm_ch1.Init.FIFOMode = DMA_FIFOMODE_DISABLE;
    
    /* TIM1_CH2 Init - clear all remaining channels */
    hdma_tim1_pwm_ch2.Instance = DMA2_Stream2;
    hdma_tim1_pwm_ch2.Init.Channel = DMA_CHANNEL_6;
    hdma_tim1_pwm_ch2.Init.Direction = DMA_MEMORY_TO_PERIPH;
    hdma_tim1_pwm_ch2.Init.PeriphInc = DMA_PINC_DISABLE;
    hdma_tim1_pwm_ch2.Init.MemInc = DMA_MINC_DISABLE;
    hdma_tim1_pwm_ch2.Init.PeriphDataAlignment = DMA_PDATAALIGN_HALFWORD;
    hdma_tim1_pwm_ch2.Init.MemDataAlignment = DMA_MDATAALIGN_HALFWORD;
    hdma_tim1_pwm_ch2.Init.Mode = DMA_CIRCULAR;
    hdma_tim1_pwm_ch2.Init.Priority = DMA_PRIORITY_VERY_HIGH;
    hdma_tim1_pwm_ch2.Init.FIFOMode = DMA_FIFOMODE_DISABLE;
     ...
     * size ...
     */
    HAL_DMA_Init(&hdma_tim1_update);
    HAL_DMA_Init(&hdma_tim1_pwm_ch1);
    HAL_DMA_Init(&hdma_tim1_pwm_ch2);

    HAL_DMA_Start(&hdma_tim1_update, (uint32_t)&ws2812_gpio_set_bits, (uint32_t)&gpio_bank->BSRR, DMA_BUFFER_SIZE);

    /* We want DMA to *clear* specific output bits, but we don't want to DMA entire 32-bit words.
     * So, point the DMA output at the upper half of BSRR, which is where the 'clear' bits are.
     */
    HAL_DMA_Start(&hdma_tim1_pwm_ch1, (uint32_t)dma_buffer, (uint32_t) &gpio_bank->BSRR + 2, DMA_BUFFER_SIZE);
    HAL_DMA_Start(&hdma_tim1_pwm_ch2, (uint32_t)&ws2812_gpio_set_bits, (uint32_t)&gpio_bank->BSRR + 2, DMA_BUFFER_SIZE);

    __HAL_TIM_ENABLE_DMA(&htimer1, TIM_DMA_UPDATE);
    __HAL_TIM_ENABLE_DMA(&htimer1, TIM_DMA_CC1);
    __HAL_TIM_ENABLE_DMA(&htimer1, TIM_DMA_CC2);
}
...
void ws2812_refresh(const struct led_channel_info *channels, GPIO_TypeDef *gpio_bank)
{
     ...
     */
	__HAL_TIM_DISABLE_DMA(&htimer1, TIM_DMA_UPDATE);
	__HAL_TIM_DISABLE_DMA(&htimer1, TIM_DMA_CC1);
	__HAL_TIM_DISABLE_DMA(&htimer1, TIM_DMA_CC2);

    __HAL_TIM_DISABLE(&htimer1);

    /* Set all LED GPIOs to 0, to begin reset pulse */
    gpio_bank->BSRR = ws2812_gpio_set_bits << 16;

    /* TODO: Retest this when I get home and have hardware with me */
#if 0
    __HAL_TIM_ENABLE(&htimer1);
     ...
     */
    for (i = 0; i < 225; i++) {
        while (!__HAL_TIM_GET_FLAG(&htimer1, TIM_FLAG_UPDATE));
        __HAL_TIM_CLEAR_FLAG(&htimer1, TIM_FLAG_UPDATE);
    }
     ...
     */
    __HAL_TIM_SET_COUNTER(&htimer1, __HAL_TIM_GET_AUTORELOAD(&htimer1) - 10);
    ...
    while(1) {
        ...
        if (!(DMA2->LISR & (DMA_LISR_TCIF1 | DMA_LISR_HTIF1))) {
            cycles++;
            continue;
        }
	...
	__HAL_DMA_DISABLE(&hdma_tim1_update);
	__HAL_DMA_DISABLE(&hdma_tim1_pwm_ch1);
	__HAL_DMA_DISABLE(&hdma_tim1_pwm_ch2);
}
